...
def load_nhtsa_data():
    ...
    df = pd.DataFrame({
        ...
        'Size': sizes, 'State': states, 'Speed': speeds,
        'Complaint_Text': [f"주행 중 {s} 증상 발생. (차종: {v}, 타이어: {b} {m})" for s,v,b,m in zip(symptoms, vehicles, brands, models)]
    })
    return df

@st.cache_data
def load_nhtsa_recalls_data():
    """NHTSA Recalls API (Recall Type = TIRE) 형태의 데이터를 모사하는 함수"""
    np.random.seed(100)
    n_recalls = 600
    dates = pd.to_datetime(np.random.choice(pd.date_range('2013-01-01', '2026-09-08'), n_recalls))
    brands = np.random.choice(['NEXEN', 'HANKOOK', 'KUMHO', 'MICHELIN', 'GOODYEAR', 'CONTINENTAL'], n_recalls, p=[0.15, 0.25, 0.15, 0.25, 0.10, 0.10])
    components = np.random.choice(['TIRES:TREAD/BELT', 'TIRES:SIDEWALL', 'TIRES:VALVE', 'TIRES:PRESSURE MONITORING'], n_recalls)
    campaign_nums = [f"{np.random.randint(13, 27)}T{np.random.randint(100, 999):03d}" for _ in range(n_recalls)]
    ...
    df_recalls = pd.DataFrame({
        'Report_Received_Date': dates,
        'Year': dates.year,
        'Manufacturer': brands,
        'Recall_Type': 'TIRE',
        'Component': components,
        'NHTSA_Campaign_Number': campaign_nums,
        'Subject': np.random.choice(subjects, n_recalls),
        'Summary': summaries
    })
    return df_recalls

df_base = load_nhtsa_data()
df_recalls_base = load_nhtsa_recalls_data()
...
# 리콜(PL) 필터
mask_recall = (df_recalls_base['Report_Received_Date'].dt.date >= st.session_state.filter_start_date) & (df_recalls_base['Report_Received_Date'].dt.date <= st.session_state.filter_end_date)
if st.session_state.filter_brands: mask_recall &= (df_recalls_base['Manufacturer'].isin(st.session_state.filter_brands))
...
col_k1.markdown(f'<div class="kpi-card"><div class="kpi-title">총 접수 건수 📄</div><div class="kpi-value">{len(df_filtered):,}</div><div class="kpi-desc">선택 기간 컴플레인 총합</div></div>', unsafe_allow_html=True)
...
col_k3.markdown(f'<div class="kpi-card"><div class="kpi-title">PL / 리콜 발생 ⚠</div><div class="kpi-value">{len(df_recalls_filtered):,}</div><div class="kpi-desc">선택 기간 리콜 건수</div></div>', unsafe_allow_html=True)
...
# ==========================================
# 10. NHTSA Recalls (PL) 현황 및 리스트 UI 함수
# ==========================================
def draw_recall_card(r):
    st.markdown(f'''
        <div class="crash-card">
            <div class="crash-title">[{r['Manufacturer']}] {r['Component']} 타이어 리콜 (PL)</div>
            <div>
                <span class="crash-meta">📅 {r['Report_Received_Date'].strftime('%Y-%m-%d')}</span>
                <span class="crash-meta">🏷 캠페인 No: {r['NHTSA_Campaign_Number']}</span>
                <span class="crash-meta">🚨 {r['Subject']}</span>
            </div>
            <div class="crash-text"><b>상세 내용:</b> {r['Summary']}</div>
        </div>
    ''', unsafe_allow_html=True)
    if r['Manufacturer'] == 'NEXEN':
        st.caption(f"NEXEN 리콜 URL (클릭하여 복사)")
        st.code(f"https://www.nhtsa.gov/recalls?nhtsaId={r['NHTSA_Campaign_Number']}", language="text")
...
if df_pl_target.empty:
    st.success(f"{target_year}년 선택 필터 내에 접수된 타이어 리콜(PL) 건수가 없습니다.")
else:
    ...
    with col_pl1:
        ...
        <div class="pl-card" style="height:100%; display:flex; flex-direction:column; justify-content:center;">
            ...
            <div style="font-size:12px; color:#A6ACAF; margin-top:5px;">NHTSA 데이터 기준</div>
        </div>
        ''', unsafe_allow_html=True)
    with col_pl2:
        fig_pl_b = px.pie(df_pl_target.groupby('Manufacturer').size().reset_index(name='Count'), names='Manufacturer', values='Count', hole=0.4, title="브랜드별 리콜 비중")
...
if all_recall_df.empty:
    ...
    for _, r in all_recall_df.head(3).iterrows(): draw_pl_card(r)
    if len(all_recall_df) > 3:
        with st.expander(f"더보기 ({len(all_recall_df)-3}건 전체 목록)"):
            with st.container(height=400):
                for _, r in all_recall_df.iloc[3:].iterrows(): draw_pl_card(r)
...
# NEXEN 우선 정렬 로직 (NEXEN이 필터에 포함되어 있으면 항상 0번 인덱스)
...
if not is_multi_brand:
    ...
    # 3개씩 묶어서 컬럼 렌더링 (NEXEN이 무조건 첫 번째에 표출됨)
    for i in range(0, len(active_brands), 3):
        ...
        for idx, b_name in enumerate(chunk):
            with cols[idx]:
                ...
                # 모델명 클릭 시 증상별 카운트 드릴다운 (Expander)
