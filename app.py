...
def load_real_nhtsa_recalls():
    """웹 브라우저로 위장하여 NHTSA 서버의 보안 차단을 우회하고 CSV를 메모리로 파싱합니다."""
    url = "https://data.transportation.gov/resource/mu99-t4jn.csv?$limit=50000"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36"
    }
    try:
        # 1. requests를 사용해 안전하게 다운로드
        res = requests.get(url, headers=headers, timeout=20)
        
        if res.status_code == 200:
            # 2. 다운로드된 텍스트를 Pandas가 읽을 수 있도록 변환
            ...
            if date_col:
                # Timestamp 형식으로 변환하여 비교 시 에러 차단
        ...
        st.error(f"네트워크 및 파싱 오류가 발생했습니다. (Error: {e})")
        return pd.DataFrame()
...
col_k2.markdown(f'<div class="kpi-card"><div class="kpi-title">최근 180일 접수 ↗</div><div class="kpi-value">{len(df_filtered[df_filtered["Date"] >= (filter_end_ts - timedelta(days=180))]):,}</div><div class="kpi-desc">최신 동향 모니터링</div></div>', unsafe_allow_html=True)
...
# ==========================================
# 10. NHTSA 공식 실시간 타이어 리콜 (PL) 현황 요약 
# ==========================================
