...
# ==========================================
# 2. 데이터 생성 (Complaints + NHTSA Recalls)
# ==========================================
...
def load_nhtsa_complaints():
    ...
    crashes = np.random.choice([0, 1], n_records, p=[0.94, 0.06])
    
    texts = [f"주행 중 {s} 증상으로 차량 통제 불능 발생. (차종: {v}, 타이어: {b} {m}, 규격: {sz}, 지역: {stt})" 
    ...
    return pd.DataFrame({
        ...
        'Size': sizes, 'State': states, 'Speed': speeds, 'Crash': crashes, 'Complaint_Text': texts
    })

@st.cache_data
def load_nhtsa_recalls():
    """NHTSA Recalls API (Recall Type = TIRE) 형태의 데이터"""
    np.random.seed(100)
    n_recalls = 800
    ...
    return pd.DataFrame({
        ...
        'Recall_Type': 'TIRE', 'Component': components, 'NHTSA_Campaign_Number': campaign_nums,
        'Subject': np.random.choice(subjects, n_recalls), 'Summary': summaries
    })

df_base = load_nhtsa_complaints()
df_recalls_base = load_nhtsa_recalls()
...
col_k3.markdown(f'<div class="kpi-card"><div class="kpi-title">타이어 리콜 (PL) ⚠</div><div class="kpi-value">{len(df_recalls_filtered):,}</div><div class="kpi-desc">NHTSA 타이어 리콜 건수</div></div>', unsafe_allow_html=True)
...
# ==========================================
# 9. NHTSA Recalls (PL) 현황 및 리스트 기능
# ==========================================
def draw_recall_card(r):
    ...
    if r['Manufacturer'] == 'NEXEN':
        st.caption("NEXEN 리콜 URL (클릭하여 복사)")
        st.code(f"https://www.nhtsa.gov/recalls?nhtsaId={r['NHTSA_Campaign_Number']}", language="text")

st.markdown('<div class="section-header" style="margin-top: 40px;">PRODUCT LIABILITY (PL) / RECALLS STATUS</div>', unsafe_allow_html=True)
...
def draw_complaint_crash_card(r):
    st.markdown(f'''
...
if all_crash_df.empty:
    ...
    for _, r in all_crash_df.head(3).iterrows(): draw_complaint_crash_card(r)
    if len(all_crash_df) > 3:
        with st.expander(f"더보기 ({len(all_crash_df)-3}건 전체 목록)"):
            with st.container(height=400):
                for _, r in all_crash_df.iloc[3:].iterrows(): draw_complaint_crash_card(r)
